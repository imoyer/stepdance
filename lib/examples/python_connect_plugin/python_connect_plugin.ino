...
void setup() {
  ...
  channel_c.begin(&output_c, SIGNAL_E); //servo motor, so we use a long pulse width
  // channel_z.set_ratio(1, 50); //straight step pass-thru.
  channel_c.set_ratio(1, 1); //straight step pass-thru.

  channel_d.begin(&output_d, SIGNAL_E);
  channel_d.set_ratio(1, 1);
  ...
  connection_generator.output_3.map(&channel_c.input_target_position);
  connection_generator.output_4.map(&channel_d.input_target_position);
