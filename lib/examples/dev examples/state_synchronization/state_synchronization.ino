...
Stepdance machine state is potentially duplicated or diffused across multiple components.

This is helpful for blending motion streams, but problematic if you want to operate in absolute coordinates.
In this example, we show how to synchronize state along a mapping chain (e.g. component A, mapped to component B, mapped to component C, etc), using push_deep() and pull_deep() blockport methods. These are internal developer methods, but functionality to work with them can be exposed by individual components like the TimeBasedInterpolator.

A part of the Mixing Metaphors Project

// (c) 2025 Ilan Moyer, Jennifer Jacobs, Emilie Yu, Devon Frost
*/
...
// #endif
  ...
  myTBI.begin();
  myTBI.output_x.map(&axidraw_kinematics.input_x);
  myTBI.output_y.map(&axidraw_kinematics.input_y);

  myRPC.begin(&Serial);
  myRPC.enroll("kinematics", axidraw_kinematics);
  myRPC.enroll("channel_a", channel_a);
  myRPC.enroll("channel_b", channel_b);
  myRPC.enroll("tbi", myTBI);
