...
void setup() {
  ...
  in_scale.begin(IO_A1);
  in_scale.set_floor(0);
  in_scale.set_ceiling(20);
  in_scale.set_deadband(1, 509, 4);
  in_scale.map(&scaling_filter.ratio);
  ...
  encoder_1.output.map(&scaling_filter.input_1);


  encoder_2.begin(ENCODER_2);
  encoder_2.set_ratio(24, 2400);
  encoder_2.invert();
  encoder_2.output.map(&scaling_filter.input_2); // map the right encoder to the y axis input of the kinematics
