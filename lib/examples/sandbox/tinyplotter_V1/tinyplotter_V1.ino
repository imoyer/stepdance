...
OutputPort output_a;  // right motor
OutputPort output_b;  // Z axis servo driver
OutputPort output_c;  // left motor
...
// -- Define Input Button --
Button button_green; //d1
Button button_yellow; //d2

AnalogInput knob_orange; //A3
AnalogInput knob_purple; //A2
...
void setup() {
  ...
  channel_a.begin(&output_c, SIGNAL_E); // Connects the channel to the "E" signal on "output_a".
  ...
  channel_b.begin(&output_a, SIGNAL_E);
  ...
  channel_z.begin(&output_b, SIGNAL_E); //servo motor, so we use a long pulse width
  ...
  encoder_1.set_ratio(24, 16384);  // 24mm per revolution, where 1 rev == 2400 encoder pulses
                                  //We're using a 600CPR encoder, which generates 4 edge transitions per cycle.
  // encoder_1.invert(); //invert the encoder direction
  encoder_1.output.map(&axidraw_kinematics.input_y);


  encoder_2.begin(ENCODER_2);
  encoder_2.set_ratio(24, 16384);
  encoder_2.invert();
  ...
  circle_gen.rotational_speed_rev_per_sec = 0;
  ...
  // -- Configure Button --
  button_green.begin(IO_D1, INPUT_PULLDOWN);
  button_green.set_mode(BUTTON_MODE_TOGGLE);
  button_green.set_callback_on_press(&pen_up);
  button_green.set_callback_on_release(&pen_down);

  button_yellow.begin(IO_D2, INPUT_PULLDOWN);
  button_yellow.set_mode(BUTTON_MODE_TOGGLE);
...
void pen_down(){
  position_gen.go(0.5, ABSOLUTE, 100);
}

void pen_up(){
  position_gen.go(6, ABSOLUTE, 100);
}
...
void report_overhead(){
  ...
  SerialUSB1.print("PURPLE KNOB REFRESH: ");
  SerialUSB1.print(knob_purple.get_refresh_rate_hz(),1);
  SerialUSB1.println("Hz");
  SerialUSB1.print("PURPLE KNOB INTERRUPT: ");
  SerialUSB1.print(knob_purple.get_interrupt_duration_us(),3);
  SerialUSB1.println("uS");


  SerialUSB1.print("ORANGE KNOB REFRESH: ");
  SerialUSB1.print(knob_orange.get_refresh_rate_hz(),1);
  SerialUSB1.println("Hz");
  SerialUSB1.print("ORANGE KNOB INTERRUPT: ");
  SerialUSB1.print(knob_orange.get_interrupt_duration_us(),3);
  SerialUSB1.println("uS");
}