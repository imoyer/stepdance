...
void setup() {
  ...
  encoder_1.output.map(&channel_b.input_target_position);


  encoder_2.begin(ENCODER_2);
  encoder_2.set_ratio(24, 16384);
  // encoder_2.invert();
  encoder_2.output.map(&channel_a.input_target_position); // map the right encoder to the y axis input of the kinematics

  // -- Configure and start EBB Interface --
  ebb_interface.begin();
  ebb_interface.output_x.map(&channel_a.input_target_position);
  ebb_interface.output_y.map(&channel_b.input_target_position);
  ...
  knob_orange.set_ceiling(20, 1020); //radius
