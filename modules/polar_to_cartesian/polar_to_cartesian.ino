...
void setup() {
  ...
  velocity_gen.begin();
  velocity_gen.map(&polar_kinematics.input_transmission_t);

  encoder_1.begin(ENCODER_1);
  encoder_1.set_ratio(2400, 25); //25mm per revolution
  encoder_1.map(&polar_kinematics.input_transmission_r);
  encoder_1.invert();

  encoder_2.begin(ENCODER_2);
  encoder_2.set_ratio(2400, 25); //1:1 angular resolution
  encoder_2.map(&polar_kinematics.input_transmission_r); //swinging pendulum
  encoder_2.invert();

  polar_kinematics.map(POLAR_OUTPUT_X, &axidraw_kinematics.input_transmission_x);
  polar_kinematics.map(POLAR_OUTPUT_Y, &axidraw_kinematics.input_transmission_y);
